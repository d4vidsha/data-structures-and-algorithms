/*
    ...
    Run with
        ./dict4 4 <input dataset> <output file> <region start_lat> \
...
int main(int argc, char **argv){
    if(argc < MINARGS){
        fprintf(stderr, "Insufficient arguments, run in form:\n"
            "\t./dict4 4 <input dataset> <output file> \\\n"
    ...
    while((query = getQuery(stdin))){
        struct queryResult *r = searchPR(dict, query, PRIMARY_KEY_FIELD);
