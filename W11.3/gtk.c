...
void setupGTKdijkstra(void *digraph, 
    ...
    gtkFreeDijkstraRes = freeDijkstraRes;
}

int **gtkGraph = NULL;
int **gtkPath = NULL;
void (*gtkFW)(int **adjMatrix, int ***pathMatrix, int size) = NULL;
void (*gtkConstructPathFW)(int **adjMatrix, int **pathMatrix, int *path, int source, 
    int destination, int size) = NULL;
void (*gtkPrintPath)(int **pathMatrix, int src, int dest) = NULL;
int fwCalculated = 0;
...
void setupGTKFW(int **graph, 
    ...
    gtkGraph = graph;
    gtkFW = fw;
    gtkConstructPathFW = constructPathFW;
    gtkPrintPath = printPath;
    fwCalculated = 0;
}

void *(*gtkDijkstraUpdate)(void *graph, void *oldRes, int edgeSource, 
        int edgeDestination, int edgeWeight);
void (*gtkAddEdge)(void *graph, int source, int destination, int weight);
int offlineEdgeCount = 0; 
...
void onlineEdgeAdd(){
    if(onlineUpdate){
        if(onlineEdges < offlineEdgeCount){
            gtkAddEdge(gtkDigraph, gtkSources[onlineEdges], gtkDestinations[onlineEdges], 
                   gtkWeights[onlineEdges]);
            if(dijkstraResults){
                /* Update all online dijkstra data structures. */
                int i;
                for(i = 0; i < gtkNodeCount; i++){
                    if(dijkstraResults[i]){
                        dijkstraResults[i] = gtkDijkstraUpdate(gtkDigraph, 
                            dijkstraResults[i], gtkSources[onlineEdges], 
                            gtkDestinations[onlineEdges], gtkWeights[onlineEdges]);
                    }
                }
            }
            onlineEdges++;
            setupGTKgraph(gtkNodeCount, onlineEdges, gtkSources, gtkDestinations, 
                gtkWeights, gtkVertexLabels);
        }
    }
}
...
void setupGTKDijkstraOnline(void *digraph, 
    ...
    gtkDijkstraUpdate = dijkstraUpdate;
    gtkAddEdge = addEdge;
    
    gtkNodeCount = nodeCount;
    offlineEdgeCount = edgeCount;
...
void update_destination(){
    ...
    if (closest_d2 < (0.01)){
        if (closest_v != dstSelected){
            ...
            int pathFW[gtkNodeCount * gtkNodeCount];
            int cost = 0;
            if(useDijkstra){
                ...
                for(i = 0; i < (gtkNodeCount * gtkNodeCount); i++){
                    pathFW[i] = 0;
                }
                if(! fwCalculated){
                    gtkFW(gtkGraph, &gtkPath, gtkNodeCount);
                    fwCalculated = 1;
                }
                gtkConstructPathFW(gtkGraph, gtkPath, pathFW, srcSelected, dstSelected, gtkNodeCount);
                for(i = 0; i < gtkEdgeCount; i++){
                    /* Get 1-D edge mapping to 2-D edges */
                    int s = gtkSources[i];
                    int d = gtkDestinations[i];
                    if(pathFW[s * gtkNodeCount + d]){
                        cost += gtkWeights[i];
                    }
                    VECTOR(shortest_path)[i] = pathFW[s * gtkNodeCount + d];
                }
            }
            
            printf("%d to %d (Cost: %d)\n", srcSelected, dstSelected, cost);
        }
    } else {
        if (dstSelected != (-1)){
            dstSelected = -1;
